...
lastDigitH len (x:y:z:xs)
            |not (z==0)=let x'=(mod x (10^len))
                            y'=(mod y (10^(len+1)))
                            y''=if and [(not (y==0)),(y'==0)] then 10^(len+1) else y' 
                        in if x'==lastDigitLen len x' y'' then x' else  lastDigitLen len x' (correct (len+1) y (lastDigitH (len+1) (y'':z:xs)))
